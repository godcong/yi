...
Usage: python scripts/build_skill.py --dist dist/ --skill-dir skills/daily-hexagram --output dist/
"""
import argparse
import os
import shutil
...
def build_generic_skill(skill_dir, output_dir):
    """Create a generic .skill file without binary (fallback for non-platform-aware installers)."""
    skill_name = "daily-hexagram.skill"
    output_path = os.path.join(output_dir, skill_name)
...
def main():
    ...
    parser.add_argument("--skill-dir", default="skills/daily-hexagram", help="Skill source dir")
    ...
    if archives:
        ...
        print("No GoReleaser archives found in dist/, skipping platform-specific builds")

    print("Building generic fallback...")
    build_generic_skill(args.skill_dir, args.output)
